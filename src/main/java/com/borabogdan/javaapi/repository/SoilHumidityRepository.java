...
public interface SoilHumidityRepository extends JpaRepository<SoilHumidityEntity, Integer> {
    ...
    @Query("SELECT new com.borabogdan.javaapi.dto.GetSoilHumidityAvgDTO( EXTRACT(DAY FROM s.timestamp), AVG(s.soilHumidity) ) FROM SoilHumidityEntity s WHERE s.timestamp >= :fromTimestamp GROUP BY 1")
